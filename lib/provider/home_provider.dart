...
class HomeProvider with ChangeNotifier {
  ...
  List<TaskModel> get filteredTasks {
    ...
    if (_selectedCategory != "All") {
      ...
      if (_selectedSubCategory != null) {
        result = result
            .where((task) => task.subCategory == _selectedSubCategory)
            .toList();
      }
    }
  ...
  void selectCategory(String category) {
    ...
    // A new main category was picked, so any previously selected
    // sub-category (which belonged to the old category) no longer applies.
    _selectedSubCategory = null;
    notifyListeners();
  }

  /// Toggle a sub-category chip: tapping the already-selected one clears it.
