...
class FirestoreService {
  ...
  Future<void> reportUser({
    ...
    debugPrint('FirestoreService.reportUser called: reporterId=$trimmedReporterId, reportedUserId=$trimmedReportedUserId, reason=$reason');
    // 1. Validate inputs
    if (trimmedReporterId.isEmpty) {
      debugPrint('FirestoreService: Reporter ID empty');
      throw Exception('Reporter ID is required.');
    }
    if (trimmedReportedUserId.isEmpty) {
      debugPrint('FirestoreService: Reported user ID empty');
      throw Exception('Reported user ID is required.');
    }
    // Optional: Validate that reporterId matches current user for security
    // Uncomment if you want to enforce that users can only report as themselves
    // if (trimmedReporterId != currentUserId) {
    //   debugPrint('FirestoreService: Reporter ID mismatch');
    //   throw Exception('You can only report as yourself.');
    // }
